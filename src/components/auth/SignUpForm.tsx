...
function getPasswordStrength(password: string): {
  ...
  const levels = [
    { score: 1, label: "Weak — add uppercase, numbers, or symbols", color: "bg-red-500"    },
...
export default function SignUpForm() {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      agreed: Yup.boolean().oneOf([true], "Please accept the terms to continue"),
    }),
    onSubmit: (values) => {
            ...
            <div>
              ...
              {formik.touched.email && formik.errors.email && (
                <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                  <span className="w-3.5 h-3.5 rounded-full border border-red-500 flex items-center justify-center text-[9px]">!</span>
                  {formik.errors.email}
                </p>
              )}
            </div>
            ...
            <div>
              <label className="flex items-start gap-2 cursor-pointer">
                ...
                <span className="text-xs text-gray-500">
                  I agree to the{" "}
                  <Link href="/terms" className="text-[#cc3602] underline">Terms of Service</Link>
                  {" "}and{" "}
                  <Link href="/privacy-policy" className="text-[#cc3602] underline">Privacy Policy</Link>
                </span>
              </label>
